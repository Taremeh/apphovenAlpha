...
export class PracticeSessionComponent implements OnInit, OnDestroy {
    ...
    public firestoreListen(): void {
        ...
        let query = pieceCollection
            .where("date", ">=",  this.firebaseRangeDate1)
            .where("date", "<=", this.firebaseRangeDate2+86399000)
            .orderBy("date", "desc");

        this.listenerUnsubscribe = query.onSnapshot((snapshot: firestore.QuerySnapshot) => {
            if (snapshot) {
                console.log("Handling Snapshot");
                console.log("SNAPSHOUT LENGTH: " + snapshot.docSnapshots.length);
    ...
    private handleSnapshot(snapshot) {
        ...
        if(snapshot.docSnapshots.length !== 0){
            ...
            // Get the amount of Practice Session Documents
            let sessionAmount = snapshot.docSnapshots.length;

            // initialize var for lastDate-Check
            let lastDate;

            // Push each Practice Session Doc into tmpResult ARray
            let tmpResult = [];
            snapshot.forEach(piece => {
                tmpResult.push(piece.data());
            });
            ...
            console.log("Firestore: SUCCESSFULL RUN-TROUGH")
        } else {
            this._ngZone.run(() => {
                this.leg1 = 0;
                this.leg2 = 0;
    ...
    deleteSession(session){
        ...
        dialogs.confirm(options).then((result: boolean) => {
            if(result){
                that._pieceService.removeSession(session.id).then(
                    function () {
                        console.log("success REMOVING");
                        /* Maintenance: Chart-Rendering Bug revealing itself
                         * when deleting Session in some cases
                         * 
                         * Under Review...
                         */
